...
async function mockApi(page: Page) {
  await page.route('**/api/workflow/**', async (route) => {
    ...
    if (url.pathname.endsWith('/definitions/page')) {
      data = {
        total: 1,
        pageNum: 1,
        pageSize: 20,
      ...
      data = {
        ...
        pageSize: 20,
        records: [
          { id: 1, tenantId: 'default', tenantCode: 'DEFAULT', tenantName: '默认租户', enabled: true },
        ],
      }
...
test('navigates across instance, assignment and tenant domains', async ({ page }) => {
  ...
  await expect(page.getByText('默认租户')).toBeVisible()
})

...
const versionForm = reactive<AgentVersionCommand>({
  ...
  outputSchema: '',
  toolCodes: [],
  retrievalProfileVersionId: undefined,
})
const {
  inputSchemaFields,
  outputSchemaFields,
...
const retrievalProfilesQuery = useQuery({
  queryKey: computed(() => ['agent-retrieval-profiles', tenantCode.value]),
  queryFn: () => agentApi.retrievalProfiles(),
  enabled: computed(() => Boolean(tenantCode.value && versionEditorVisible.value)),
})
...
function openVersionEditor(version: AgentVersion) {
  ...
  Object.assign(versionForm, {
    ...
    retrievalProfileVersionId: version.retrievalProfileVersionId,
  })
  ...
  <div class="management-page page-stack agent-management-page">
    ...
    >
      <el-form class="dialog-form" label-position="top">
        ...
        <el-form-item v-if="versionForm.executionMode === 'PLATFORM_AGENT'" label="工具集">
          <el-select
          ...
          >
            <el-option label="知识检索 · 平台内置" value="knowledge_search">
          ...
          <div class="form-help">
            平台内置只读工具与已审核发布的 MCP 快照均可选择。工具集合会随 Agent
            版本冻结，流程节点只能继续缩小范围。
          </div>
        </el-form-item>
        <el-alert
          ...
          :closable="false"
          title="暂无已发布 MCP 工具"
          description="当前仍可使用平台内置工具；如需外部工具，请先在工具目录中完成发现和审核发布。"
        />
        <el-form-item
          v-if="
            versionForm.executionMode === 'PLATFORM_AGENT' &&
            versionForm.toolCodes?.includes('knowledge_search')
          "
          label="检索 Profile"
          required
        >
          <el-select
            v-model="versionForm.retrievalProfileVersionId"
            filterable
            clearable
            :disabled="editingVersion?.status === 'PUBLISHED'"
            :loading="retrievalProfilesQuery.isFetching.value"
            placeholder="选择已发布的检索 Profile"
          >
            <el-option
              v-for="profile in retrievalProfilesQuery.data.value?.records || []"
              :key="profile.id"
              :label="`${profile.profileCode} · 第 ${profile.version} 版`"
              :value="profile.id"
            />
          </el-select>
          <div class="form-help">
            Agent 运行时只使用此处冻结的已发布 Profile；模型不能自行扩大知识范围。
          </div>
        </el-form-item>
        <el-alert
          v-if="
            versionForm.executionMode === 'PLATFORM_AGENT' &&
            versionForm.toolCodes?.includes('knowledge_search') &&
            !retrievalProfilesQuery.data.value?.records?.length
          "
          type="warning"
          :closable="false"
          title="暂无已发布检索 Profile"
          description="请先在知识库管理中完成文档摄取、创建并发布 Profile，再绑定 knowledge_search。"
        />
